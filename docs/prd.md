...
| ------------ | ---------------------------- | ------------ | ---------------------- | ----------------------------------------------------------------------------- |
| 首页         | `/`                          | ISR          | 已落地                 | 聚合 PBE 新增皮肤、英雄入口、皮肤系列、皮肤宇宙和臻彩皮肤入口。               |
...
| 臻彩皮肤     | `/prestige-chromas`          | ISR          | 已落地入口页           | 作为首页路由级 Tab 展示臻彩皮肤列表，点击进入独立详情页；更新记录后续补充。   |
...
- 如果某个炫彩 ID 能在臻彩皮肤接口中匹配到对应记录，则该炫彩缩略图在配色组件上方展示臻彩 tag 图标；tag 图标无背景，按资源原图尺寸显示，hover 或 focus 时展示中文分类名称，点击 tag 跳转到对应臻彩详情页，不影响点击缩略图切换炫彩。
- 带臻彩关联的炫彩项默认显示臻彩图片；切换到聚焦模式时显示原炫彩图片。
- 如果当前选中的炫彩能匹配到臻彩皮肤记录，则皮肤详情页“关联内容”中仅展示该炫彩对应的臻彩皮肤，并支持点击跳转到臻彩详情页。
...
- 各 tab 路由与内容对应关系：
  ...
  | /prestige-chromas | 臻彩皮肤 | 臻彩皮肤卡片网格 + 分组/排序控制             |
  | /coming           | 后续内容 | 占位卡片                                     |
...
- 图标语义统一为：英雄使用用户、皮肤宇宙使用轨道、皮肤套装使用地球、臻彩皮肤使用调色板、后续内容使用闪光；Tab、臻彩皮肤分组按钮与臻彩详情页返回臻彩皮肤列表入口均从 `src/lib/navigation/content-sections.ts` 读取共享图标配置，后续调整只修改共享配置。
- 臻彩皮肤 Tab 支持「所有」不分组展示，以及按英雄、皮肤宇宙、皮肤套装分组展示；分组选项顺序固定为「所有、英雄、皮肤宇宙、皮肤套装」；默认选择「所有」并直接输出卡片网格，不展示分组标题。
- 点击「所有」时需要清空数量排序等分组相关条件，只保留不分组列表所需的上线时间排序。
- 「臻彩分类」作为筛选条件展示在排序控件的「数量」后面，分类来源为接口 `cid/cname`；筛选条件只过滤列表，不作为分组选项。
- 臻彩皮肤 Tab 的「上线时间」排序实际使用接口 `rank` 字段；默认按 `rank` 降序排序，支持升序/降序切换，排序控件展示方式参考皮肤套装列表页。
- 臻彩皮肤 Tab 选择任意分组时，同时提供「数量」排序选项，用于按分组内臻彩数量升序/降序排列分组；组内卡片仍按 `rank` 排序。
...
- 详情抽屉展示臻彩名称；臻彩描述需等待接口返回专属描述字段后再展示，当前不使用拼接摘要作为可见描述。
- 臻彩资料仅展示炫彩 ID 和臻彩分类；缺失字段直接省略，不伪造默认说明。
- 臻彩详情抽屉不展示普通皮肤详情的“炫彩外观”折叠块。
- 臻彩分类内容需要显示中文分类名称；如果接口返回 `timgUrl`，悬浮或聚焦中文分类名称时显示对应 tag 图标预览，不常驻展示额外小图标；点击中文分类名称跳转到 `/prestige-chromas` 并通过 `category` query 自动筛选该分类内容。
- 臻彩详情页异步查询同英雄皮肤列表，并用臻彩 `skinId` 匹配普通皮肤炫彩 ID；匹配成功时，在“关联内容”中展示所属普通皮肤并支持跳转到对应皮肤详情页。
- 标题区域的 tag 图标保持小尺寸展示；悬浮或聚焦该图标时，预览图按资源原始尺寸展示，仅在超出视口时做最大宽高约束。
...
- title 使用“`{itemName}臻彩皮肤与资料`”；description 由英雄、分类、系列、宇宙和上线时间等真实字段组合。
- 页面输出臻彩专属 `ImageObject` 与 `BreadcrumbList`；面包屑至少包含首页、臻彩皮肤和当前臻彩。
...
- PC 端详情页右上角的查看、切换、下载和详情入口整体归属同一个按钮组；皮肤信息按钮不额外使用独立外观样式，移动端维持既有详情入口布局。
