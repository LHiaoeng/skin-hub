...
export async function generateStaticParams() {
  ...
  return skinlines.map((skinline) => ({
    skinlineId: skinlinePath(skinline).replace("/skinlines/", ""),
  }));
}

export async function generateMetadata({
  params,
}: SkinlineDetailPageProps): Promise<Metadata> {
  ...
  const description =
    ...
    `探索 ${skinline.name} 皮肤套装，浏览其下的所有皮肤。`;
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      images: imageUrl
        ? [{ url: imageUrl, alt: `${skinline.name} 皮肤套装原画` }]
        : undefined,
    },
  };
}

export default async function SkinlineDetailPage({
  params,
  searchParams,
}: SkinlineDetailPageProps) {
  const { skinlineId } = await params;
  const id = parseValidId(skinlineId);
  if (!id) notFound();
  ...
  const breadcrumbItems = [
    ...
    ...(skinline.universe
      ? [
          ...
          {
            name: skinline.universe.name,
            url: `${siteUrl}${universePath(skinline.universe)}`,
          },
        ]
    ...
    <>
            ...
            ? [
                {
                  label: "皮肤宇宙",
                  href: "/universes",
                  sectionKey: "universes" as const,
                },
                {
                  label: skinline.universe.name,
                  href: universePath(skinline.universe),
                  sectionKey: "universes" as const,
                },
              ]
            : [
      ...
      >
        <SkinSortToolbar
          basePath={basePath}
          activeSort={sort}
          totalLabel={`${sortedSkins.length} 款皮肤`}
        />
...
function getSkinlineBackgrounds(skinline: SkinlineDetail): string[] {
  const skin = skinline.skins.find(
    (item) => item.uncenteredSplashPath || item.splashPath,
  );
  return uniqueUrls([
